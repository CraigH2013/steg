...
function getAlter(pitch) {
    ...
    if (alter === undefined || alter === null) {
        return pitch_1.Alter.NATURAL;
    }
    if (alter === 0) {
        return pitch_1.Alter.NATURAL;
    }
    else if (alter > 0) {
        return pitch_1.Alter.SHARP;
    }
    else {
        return pitch_1.Alter.FLAT;
    }
}
/**
...
function getStep(step) {
    switch (step.toUpperCase()) {
        case "A":
            return pitch_1.Step.A;
        case "B":
            return pitch_1.Step.B;
        case "C":
            return pitch_1.Step.C;
        case "D":
            return pitch_1.Step.D;
        case "E":
            return pitch_1.Step.E;
        case "F":
            return pitch_1.Step.F;
        case "G":
            return pitch_1.Step.G;
    }
}
/**
 ...
 */
function getPitchedNote(note, divisions, duration) {
 ...
 */
function getRestedNote(divisions, duration) {
    return {
        chord: false,
        pitch: {
            step: pitch_1.Step.REST,
            alter: pitch_1.Alter.REST,
            octave: 0
        },
        rest: true,
        divisions,
 ...
 */
function parseNote(note, divisions) {
 ...
 */
function parseNotes(notes, divisions) {
...
function getNotes(measures) {
    ...
    for (let i = 0; i < measures.length; i++) {
        ...
        if (divisions === null) {
            throw new Error("A divisions value could not be found on the first measure");
        }
...
function parse(data) {
    return __awaiter(this, void 0, void 0, function* () {
        const json = yield xmlToJson(data);
